...
// dequantize a quantized KV cache tensor to contiguous F16 scratch for the FA-RDNA2 kernels
...
// F32 KV side -> contiguous F16 scratch (same layout as the quant prepass above).
// One thread per element; host dispatches ne0*ne1*ne2*ne3 threads (blck_size 1).
kernel void kernel_flash_attn_ext_kv_f32_f16(
        constant ggml_metal_kargs_flash_attn_ext_kv_f16 & args,
        device const char * x,
        device       half * x_dst,
        uint gid [[thread_position_in_grid]]) {
    const uint64_t n = (uint64_t) args.ne0*args.ne1*args.ne2*args.ne3;
    if (gid >= n) {
        return;
    }

    const uint i0 = gid % (uint) args.ne0;
    uint ib       = gid / (uint) args.ne0;
    const uint i1 = ib % (uint) args.ne1;
    ib /= (uint) args.ne1;
    const uint i2 = ib % (uint) args.ne2;
    const uint i3 = ib / (uint) args.ne2;

    const uint64_t offs = (uint64_t) i0*args.nb0 + (uint64_t) i1*args.nb1 + i2*args.nb2 + i3*args.nb3;

    x_dst[gid] = (half) (*(device const float *) (x + offs));
}
...
kernel void kernel_flash_attn_ext_amd_tile_dk(
    ...
    // ── 输出：out = o / l（dst 与 Q 布局不同：dst 是 [DV,H,N,B]，Q 是 [DK,N,H,B]）──
    if (r_valid) {
